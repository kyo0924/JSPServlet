...
public class ManageMember extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (pageNo == 1) {
			...
			pageBar += "<a href='" + request.getContextPath() + "/sample?cPage=" + (pageNo - 1) + "&numPerPage=" + numPerPage + "'>[이전]</a>";
		}
		...
		while(!(pageNo > pageEnd || pageNo > totalPage)) {
			if (cPage == pageNo) {
				...
				pageBar +="<a href='" + request.getContextPath() + "/sample?cPage=" + pageNo + "&numPerPage=" + numPerPage +"'>" + pageNo + "</a>";
			}
			pageNo++;
		}
